...
import { DEFAULT_OAUTH_CALLBACK_PORT, startOAuthCallbackServer } from '../lib/oauth-server.js'
    ...
    auth.command('login')
        ...
        .action(
            async (options: {
                ...
                if (!url) {
                    const baseUrlInput = await prompt(`Base URL (default: ${configuredBaseUrl}): `)
                ...
                if (!clientId) {
                    console.error(
                        formatError('OAUTH_CLIENT_ID_REQUIRED', 'OAuth client ID is required.', [
                ...
                let callbackServer: Awaited<ReturnType<typeof startOAuthCallbackServer>>
                try {
