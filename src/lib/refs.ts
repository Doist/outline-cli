...
function formatSuggestions<T>(
    ...
    return suggestions.map((item) => `  - "${getName(item)}" (${getDisplayId(item)})`).join('\n')
}
 ...
 */
async function resolveRef<T extends { id: string }>(options: ResolveRefOptions<T>): Promise<T> {
    const { ref, fetchById, fetchAllPaginated, getName, getDisplayId, entityType, extractSlugId } =
        options
    ...
    const exactMatches = items.filter((item) => getName(item).toLowerCase() === refLower)
    ...
    // Try partial match
    const partialMatches = items.filter((item) => getName(item).toLowerCase().includes(refLower))
    ...
    if (partialMatches.length > 1) {
        const suggestions = formatSuggestions(partialMatches, getName, getDisplayId)
        throw new Error(`Ambiguous ${entityType} reference "${ref}". Did you mean:\n${suggestions}`)
    }
